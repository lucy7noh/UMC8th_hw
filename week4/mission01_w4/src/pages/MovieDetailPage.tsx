...
const MovieDetailPage = (): Element => {
  ...
  const { isPending, isError, data: movie } = useCustomFetch<MovieDetail>(url, "ko-KR");
  ...
  const crew = movie.credits?.crew?.filter((person) => person.job === "Director") || [];
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="flex flex-col md:flex-row gap-6 items-start bg-white/80 backdrop-blur-sm p-6 rounded-xl shadow-xl">
          ...
          <div className="flex-1">
            ...
            <ul className="text-xl text-gray-600 space-y-1 mt-20">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
              {crew.map((director) => (
                <div key={director.id} className="text-center text-sm">
                  <img
                    className="w-30 h-40 object-cover rounded-full mx-auto border-4 border-white"
